...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal):
        plane_eq_expanded = torch.repeat_interleave(plane_eq, int(self.upratio), 2) # [B, 4, H*upratio, W]
        plane_eq_expanded = torch.repeat_interleave(plane_eq_expanded, int(self.upratio), 3) # [B, 4, H*upratio, W*upratio]
        n1 = plane_eq_expanded[:, 0, :, :] # [B, H*upratio, W*upratio]
        n2 = plane_eq_expanded[:, 1, :, :] # [B, H*upratio, W*upratio]
        n3 = plane_eq_expanded[:, 2, :, :] # [B, H*upratio, W*upratio]
        n4 = plane_eq_expanded[:, 3, :, :] # [B, H*upratio, W*upratio]
        
        u = self.u.repeat(plane_eq.size(0), plane_eq.size(2) * int(self.upratio), plane_eq.size(3)).cuda() # [B, H*upratio, W*upratio]
        u = (u - (self.upratio - 1) * 0.5) / self.upratio # [B, H*upratio, W*upratio]
        
        v = self.v.repeat(plane_eq.size(0), plane_eq.size(2), plane_eq.size(3) * int(self.upratio)).cuda() # [B, H*upratio, W*upratio]
        v = (v - (self.upratio - 1) * 0.5) / self.upratio # [B, H*upratio, W*upratio]

        return n4 / (n1 * u + n2 * v + n3) # [B, H*upratio, W*upratio]
...
class bts(nn.Module):
    ...
    def forward(self, features, focal):
        skip0, skip1, skip2, skip3 = features[0], features[1], features[2], features[3] # [B, 64, H/2, W/2], [B, 256, H/4, W/4], [B, 512, H/8, W/8], [B, 1024, H/16, W/16]
        dense_features = torch.nn.ReLU()(features[4]) # [B, 2048, H/32, W/32]
        upconv5 = self.upconv5(dense_features) # [B, N, H/16, W/16]  ***N = 512 (default)***
        upconv5 = self.bn5(upconv5)  # [B, N, H/16, W/16]
        concat5 = torch.cat([upconv5, skip3], dim=1) # [B, N+1024, H/16, W/16]
        iconv5 = self.conv5(concat5) # [B, N, H/16, W/16]

        upconv4 = self.upconv4(iconv5) # [B, N/2, H/8, W/8]
        upconv4 = self.bn4(upconv4) # [B, N/2, H/8, W/8]
        concat4 = torch.cat([upconv4, skip2], dim=1) # [B, N/2+512, H/8, W/8]
        iconv4 = self.conv4(concat4) # [B, N/2, H/8, W/8]
        iconv4 = self.bn4_2(iconv4) # [B, N/2, H/8, W/8]


        # Dense Atrous Spatial Pyramid Pooling (DASPP)
        daspp_3 = self.daspp_3(iconv4) # [B, N/4, H/8, W/8]
        concat4_2 = torch.cat([concat4, daspp_3], dim=1) # [B, N/2+512+N/4, H/8, W/8]
        daspp_6 = self.daspp_6(concat4_2) # [B, N/4, H/8, W/8]
        concat4_3 = torch.cat([concat4_2, daspp_6], dim=1) # [B, N/2+512+N/4+N/4, H/8, W/8] 
        daspp_12 = self.daspp_12(concat4_3) # [B, N/4, H/8, W/8]
        concat4_4 = torch.cat([concat4_3, daspp_12], dim=1) # [B, N/2+512+N/4+N/4+N/4, H/8, W/8]
        daspp_18 = self.daspp_18(concat4_4) # [B, N/4, H/8, W/8] 
        concat4_5 = torch.cat([concat4_4, daspp_18], dim=1) # [B, N/2+512+N/4+N/4+N/4+N/4, H/8, W/8]
        daspp_24 = self.daspp_24(concat4_5) # [B, N/4, H/8, W/8]
        concat4_daspp = torch.cat([iconv4, daspp_3, daspp_6, daspp_12, daspp_18, daspp_24], dim=1) # [B, N/2+N/4+N/4+N/4+N/4+N/4, H/8, W/8]
        daspp_feat = self.daspp_conv(concat4_daspp) # [B, N/4, H/8, W/8]
        
        reduc8x8 = self.reduc8x8(daspp_feat) # [B, 4, H/8, W/8]
        plane_normal_8x8 = reduc8x8[:, :3, :, :] # [B, 3, H/8, W/8]
        plane_normal_8x8 = F.normalize(plane_normal_8x8, 2, 1) # [B, 3, H/8, W/8]
        plane_dist_8x8 = reduc8x8[:, 3, :, :] # [B, H/8, W/8]
        ...
        depth_8x8_scaled = depth_8x8.unsqueeze(1) / self.params.max_depth # [B, 1, H, W]
        depth_8x8_scaled_ds = F.interpolate(depth_8x8_scaled, scale_factor=0.25, mode='nearest') # [B, 1, H/4, W/4]
        
        upconv3 = self.upconv3(daspp_feat) # [B, N/4, H/4, W/4]
        upconv3 = self.bn3(upconv3) # [B, N/4, H/4, W/4]
        concat3 = torch.cat([upconv3, skip1, depth_8x8_scaled_ds], dim=1) #[B, N/4+256+1, H/4, W/4]
        iconv3 = self.conv3(concat3) # [B, N/4, H/4, W/4]
        
        reduc4x4 = self.reduc4x4(iconv3) # [B, 4, H/4, W/4]
        plane_normal_4x4 = reduc4x4[:, :3, :, :] # [B, 3, H/4, W/4]
        plane_normal_4x4 = F.normalize(plane_normal_4x4, 2, 1) # [B, 3, H/4, W/4]
        plane_dist_4x4 = reduc4x4[:, 3, :, :] # [B, H/4, W/4]
        plane_eq_4x4 = torch.cat([plane_normal_4x4, plane_dist_4x4.unsqueeze(1)], 1) # [B, 4, H/4, W/4]
        depth_4x4 = self.lpg4x4(plane_eq_4x4, focal) # [B, H, W]
        depth_4x4_scaled = depth_4x4.unsqueeze(1) / self.params.max_depth # [B, 1, H, W]
        depth_4x4_scaled_ds = F.interpolate(depth_4x4_scaled, scale_factor=0.5, mode='nearest') # [B, 1, H/2, W/2]
        
        upconv2 = self.upconv2(iconv3) # [B, N/8, H/2, W/2]
        upconv2 = self.bn2(upconv2) # [B, N/8, H/2, W/2]
        concat2 = torch.cat([upconv2, skip0, depth_4x4_scaled_ds], dim=1) # [B, N/8+64+1, H/2, W/2]
        iconv2 = self.conv2(concat2) # [B, N/8, H/2, W/2]
        
        reduc2x2 = self.reduc2x2(iconv2) # [B, 4, H/2, W/2]
        plane_normal_2x2 = reduc2x2[:, :3, :, :] # [B, 3, H/2, W/2]
        plane_normal_2x2 = F.normalize(plane_normal_2x2, 2, 1) # [B, 3, H/2, W/2]
        plane_dist_2x2 = reduc2x2[:, 3, :, :] # [B, H/2, W/2]
        plane_eq_2x2 = torch.cat([plane_normal_2x2, plane_dist_2x2.unsqueeze(1)], 1) # [B, 4, H/2, W/2]
        depth_2x2 = self.lpg2x2(plane_eq_2x2, focal) # [B, H, W]
        depth_2x2_scaled = depth_2x2.unsqueeze(1) / self.params.max_depth # [B, 1, H, W]
        
        upconv1 = self.upconv1(iconv2) # [B, N/16, H, W]
        reduc1x1 = self.reduc1x1(upconv1) # [B, 1, H, W]
        concat1 = torch.cat([upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled], dim=1) # [B, N/16+1+1+1+1, H, W]
        iconv1 = self.conv1(concat1) # [B, N/16, H, W]
        final_depth = self.params.max_depth * self.get_depth(iconv1) # [B, 1, H, W]
        if self.params.dataset == 'kitti':
            final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873
        
        return depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, reduc1x1, final_depth # [B, 1, H, W] for each scale and final depth
...
class encoder(nn.Module):
    def __init__(self, params):
        ...
        self.params = params
        if params.encoder == 'resnet50_bts':
            ...
            self.base_model = models.resnext50_32x4d(wqeights=models.ResNeXt50_32X4D_Weights.DEFAULT)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
