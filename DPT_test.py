...
class AddReadout(nn.Module):
    """Suma (broadcast) el CLS (o medio CLS+DIST) a todos los tokens de parches."""
    # [B, 1+N, C] -> [B, N, C]
    def __init__(self, start_index=1):
...
def _make_vit_b16_backbone(
    ...
    pretrained.act_postprocess1 = nn.Sequential(
        readout_oper[0], # [B, N, 1024]
        Transpose(1, 2), # [B, 1024, N]
        nn.Unflatten(2, torch.Size([size[0] // 16, size[1] // 16])), # [B, 1024, H/16, W/16]
        nn.Conv2d(in_channels=vit_features, out_channels=features[0], kernel_size=1,stride=1, padding=0), # [B, 256, H/16, W/16]
        nn.ConvTranspose2d(in_channels=features[0], out_channels=features[0], kernel_size=4, stride=4, padding=0, bias=True, dilation=1, groups=1), # [B, 256, H/4, W/4]
    )

    pretrained.act_postprocess2 = nn.Sequential(
        readout_oper[1], # [B, N, 1024]
        Transpose(1, 2), # [B, 1024, N]
        nn.Unflatten(2, torch.Size([size[0] // 16, size[1] // 16])), # [B, 1024, H/16, W/16]
        nn.Conv2d(in_channels=vit_features, out_channels=features[1], kernel_size=1, stride=1, padding=0), # [B, 512, H/16, W/16]
        nn.ConvTranspose2d(in_channels=features[1], out_channels=features[1], kernel_size=2, stride=2, padding=0, bias=True, dilation=1, groups=1), # [B, 512, H/8, W/8]
    )

    pretrained.act_postprocess3 = nn.Sequential(
        readout_oper[2], # [B, N, 1024]
        Transpose(1, 2), # [B, 1024, N]
        nn.Unflatten(2, torch.Size([size[0] // 16, size[1] // 16])), # [B, 1024, H/16, W/16]
        nn.Conv2d(in_channels=vit_features, out_channels=features[2], kernel_size=1, stride=1, padding=0), # [B, 1024, H/16, W/16]
    )

    pretrained.act_postprocess4 = nn.Sequential(
        readout_oper[3], # [B, N, 1024]
        Transpose(1, 2), # [B, 1024, N]
        nn.Unflatten(2, torch.Size([size[0] // 16, size[1] // 16])), # [B, 1024, H/16, W/16]
        nn.Conv2d(in_channels=vit_features, out_channels=features[3], kernel_size=1, stride=1, padding=0), # [B, 1024, H/16, W/16]
        nn.Conv2d(in_channels=features[3], out_channels=features[3], kernel_size=3, stride=2, padding=1), # [B, 1024, H/32, W/32]
    )
...
def _make_pretrained_vitl16_384(
    ...
    return _make_vit_b16_backbone(
        model,
        features=[96, 192, 384, 768], # [256, 512, 1024, 1024], # Usar si ViT-B/16
        hooks=hooks,
...
class FeatureFusionBlock(nn.Module):
    """
    Fusiona (opcional) un skip con el camino top-down y upsamplea ×2.
    """
    def __init__(self, features: int):
        super().__init__()
...
class DPTDepthModel(nn.Module):
    def __init__(self,
        ...
        """
        features: ancho F tras 'scratch' (256 típico)
    ...
    def forward(self, x: torch.Tensor):                   # x: [B,3,H,W]
        ...
        # l1:[B,256,H/4,W/4], l2:[B,512,H/8,W/8], l3:[B,1024,H/16,W/16], l4:[B,1024,H/32,W/32]
